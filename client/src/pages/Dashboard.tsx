...
export default function Dashboard() {
  ...
  if (!summary) {
    return <div className="text-mine-300">Loading dashboard…</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-mine-300 text-sm">Live status across all mine sites</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
        <StatCard label="Sites" value={counts.siteCount} />
        <StatCard label="Sensors" value={counts.sensorCount} />
        <StatCard label="Open Alerts" value={counts.openAlerts} tone={counts.openAlerts > 0 ? "hazard" : undefined} />
        <StatCard label="Critical" value={counts.criticalAlerts} tone={counts.criticalAlerts > 0 ? "danger" : undefined} />
        <StatCard label="On Shift" value={counts.onShiftWorkers} />
        <StatCard label="Open Incidents" value={counts.openIncidents} tone={counts.openIncidents > 0 ? "hazard" : undefined} />
        <StatCard label="Equipment Down" value={counts.equipmentDown} tone={counts.equipmentDown > 0 ? "danger" : undefined} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className={`${cardClass} lg:col-span-2 p-5`}>
          <h2 className="text-sm font-semibold mb-4">Site Status</h2>
        ...
        <div className={`${cardClass} p-5`}>
          <h2 className="text-sm font-semibold mb-4">Active Alerts</h2>
          <div className="space-y-3 max-h-[560px] overflow-y-auto">
            {recentAlerts.length === 0 && (
              <div className="text-mine-400 text-sm">No open alerts. All clear.</div>
            )}
