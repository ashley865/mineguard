...
export default function DueDate({ value, withinDays = 30 }: { value?: string | null; withinDays?: number }) {
  ...
  if (!value) return <span className="text-hazard-500">{t("resourceGovernance.notSet")}</span>;
  const days = daysUntil(value);
  const label = new Date(value).toLocaleDateString();
  if (days != null && days < 0) {
    return (
      <span className="text-danger-500 font-semibold">
        {label} <span className="text-[11px] font-normal">({t("resourceGovernance.overdueByDays", { count: Math.abs(days) })})</span>
      </span>
    );
  }
  if (days != null && days <= withinDays) {
    return (
      <span className="text-hazard-500">
        {label} <span className="text-[11px]">({t("resourceGovernance.inDays", { count: days })})</span>
      </span>
    );
  }
