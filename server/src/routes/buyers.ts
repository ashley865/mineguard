...
import { sendBuyerVerificationEmail } from "../lib/emailVerification";
...
const registerSchema = z.object({
  ...
  // Honeypot: a hidden field no real applicant ever fills in — see isHoneypotFilled.
...
router.post("/register", authLimiter, upload.array("documents", 6), async (req, res) => {
  ...
  const { website: _website, turnstileToken: _turnstileToken, ...buyerData } = parsed.data;
  ...
  // is clicked (see lib/emailVerification.ts) — buyerAuth.ts's login refuses an unverified
  // account, so a typo'd or unowned email can never reach the marketplace, on top of the
  // staff APPROVED review every buyer still needs regardless.
  await sendBuyerVerificationEmail(buyer.id, buyer.contactEmail, buyer.contactName);
  await recordPublicSubmission("BUYER_REGISTER", req.ip, null);
  res.status(201).json({ pendingVerification: true, email: buyer.contactEmail });
});
...
router.post("/", upload.array("documents", 6), async (req, res) => {
  ...
  const buyer = await prisma.buyer.create({
    data: {
      ...buyerData,
      // Staff creating this record has already vetted the buyer directly, unlike public
      // self-registration above — trusted immediately rather than requiring an email click.
